...
vec3 computeSpotLight(SpotLight light, vec3 surfel_to_camera)
{
    ...
    float theta = dot(surfel_to_light, -light.spotDirection);
    float intensity = clamp((cos(theta)-cos(light.outerCutOff))/(cos(light.innerCutOff)-cos(light.outerCutOff)), 0, 1);
